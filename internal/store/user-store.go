...
func (s *UserStore) GetUserByUsername(username string) (*model.User, error) {
	var u model.User
	err := s.DB.QueryRow(`SELECT id, username, hashed_password, created_at FROM users WHERE username = ?`, username).Scan(&u.ID, &u.Username, &u.HashedPassword, &u.HashedPassword)
